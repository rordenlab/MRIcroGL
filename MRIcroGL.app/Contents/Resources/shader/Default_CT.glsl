...
void main() {
	...
	while (lengthAcc <= len) {
		float alpha = texture(intensityVol,samplePos).a;
		alpha = 1.0-pow((1.0 - alpha), stepSize/sliceSize);
		alphaAcc = (1.0 - alphaAcc) * alpha + alphaAcc;
		if (alphaAcc > alphaThresh )
			break;
		samplePos += deltaDir;
		lengthAcc += stepSize;
	} //while lengthAcc < len
	if (lengthAcc < len) {
		...
		colAcc = vec4(colorSample.rgb, alphaAcc);
