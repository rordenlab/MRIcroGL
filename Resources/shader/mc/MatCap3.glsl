...
overlayDepth|float|0.0|0.2|0.8
//vert
#version 330 core
...
void main() {
	...
	//overlay pass
	float minAlpha = 0.0;
	vec4 ocolorSample = vec4(0.0,0.0,0.0,0.0);
	if ( overlays > 0 ) {
		...
		while (lengthAcc <= len) {
			...
			if (colorSample.a > 0.00) {
				...
				ocolorSample = colorSample;

			}
			ocolorSample.a = max(colorSample.a, minAlpha);
	...
	while (lengthAcc <= len) {
		...
		if (colorSample.a > 0.01) {
			...
			if (lengthAcc > stepSizeX2) {
				...
				//dark edges parallel with viewing angle
				float lightNormDot = dot(gradSample.rgb, rayDir);
