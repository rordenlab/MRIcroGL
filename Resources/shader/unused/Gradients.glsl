...
diffuse|float|0.0|1.0|1|Illuminate surface based on light position
...
void main() {
	...
	while (lengthAcc <= len) {
		...
		vec3 grad = texture(gradientVol, samplePos).rgb;
		grad = abs((grad * 2.0) - 1.0);
		colorSample.rgb = grad;
		//colorSample.rgb = lightPosition;
		//vec3 lightdir = dir;
		//colorSample.rgb = 0.5*(lightdir+1.0);
		//colorSample.rgb = lightPosition;
		colorSample.a = 1.0-pow((1.0 - colorSample.a), stepSize/sliceSize);
		vec3 a = colorSample.rgb * ambient;
		float s =  0;
