...
void main() {
	...
	vec3 startPos = start.xyz;
	if (doJitter > 0.5)
		startPos += deltaDir* (fract(sin(gl_FragCoord.x * 12.9898 + gl_FragCoord.y * 78.233) * 43758.5453));
	//overlay pass
	...
	if ( overlays > 0.5 ) {
		...
		if (overlayClip > 0) {
			...
			stepSizeX2 = lengthAcc + (stepSizeX * 2.0);
			len = lenClip;
		}
		while (lengthAcc <= len) {
	...
	while (lengthAcc <= len) {
		...
		colorSample.a = 1.0-pow((1.0 - colorSample.a), stepSizeX/sliceSize);
		if (showGradient > 0.5) {
			colorSample.rgb = abs(texture(gradientVol,samplePos).rgb);
		...
		if (colorSample.a > 0.01) {
			...
			if (showGradient > 0.5)
				colorSample.rgb = abs(gradSample.rgb);

				a = colorSample.rgb * ambient;
			if (lengthAcc > stepSizeX2) {
				float lightNormDot = dot(gradSample.rgb, lightPosition);
				d = max(lightNormDot, 0.0) * colorSample.rgb * diffuse;
				s =   specular * pow(max(dot(reflect(lightPosition, gradSample.rgb), dir), 0.0), shininess);
