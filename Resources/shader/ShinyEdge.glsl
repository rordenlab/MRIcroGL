//pref
ambient|float|0.0|0.7|1|Illuminate surface regardless of lighting
diffuse|float|0.0|0.4|1|Illuminate surface based on light position
	...
	while (samplePos.a <= len) {
		...
		if (colorSample.a > 0.01) {
			...
			if (samplePos.a > stepSizeX2) {
				...
				float edge = dot(gradSample.rgb, lightDirHeadOn); //with respect to viewer
				//edge = smoothstep(0.0, 0.5, edge);
				edge = mix(0.0, 3.0, edge);
				
				edge = clamp(edge, 0.0, 1.0);
				edge = (edge / ((((1.0/edgeBias) - 2.0)*(1.0 - edge))+1.0));
				colorSample.rbg *= edge;
